...
void printTelemetry(const TelemetryPacket &t)
{
  ...
  Serial.print(t.pwm_fwd); Serial.print(",");
  Serial.print(t.pwm_rev); Serial.print(",");
  Serial.print(t.pwm_lft); Serial.print(",");
  Serial.print(t.pwm_rgt); Serial.print(",");
...
void sendCommand(float fwd, float rev, float lft, float rgt, int rw)
{
  ...
  pkt.fwd = (uint16_t)(clamp01(fwd) * 1000.0f);
  pkt.rev = (uint16_t)(clamp01(rev) * 1000.0f);
  pkt.lft = (uint16_t)(clamp01(lft) * 1000.0f);
  pkt.rgt = (uint16_t)(clamp01(rgt) * 1000.0f);
...
void parseLine(char* line)
{
  ...
  if (parsed == 5) {
    sendCommand(fwd, rev, lft, rgt, rw);
