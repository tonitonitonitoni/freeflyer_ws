...
# Step 1 — frames → observations

def extract_frame_observations_from_dirs(frames, pose_log):
    """
    frames[idx]   = BGR image
    pose_log[idx] = (cam_pos_w, yaw_rad)
    """
    observations = []
    ...
    for idx in sorted(frames.keys()):
        ...
        cam_pos_w, yaw = pose_log[idx]

        # Beacon → camera offset (same as before)
        t_bc = np.array([0.12, 0.00, 0.05])
        cam_pos_w = cam_pos_w + R_wc_from_yaw(yaw) @ t_bc
...
def build_starfield_map_from_logged_data(
    frames_dir,
    poses_dir,
    K,
    dist,
    ...
    # Load synced data
    # -----------------------------
    frames, pose_log = load_logged_dataset(
        frames_dir,
        poses_dir,
    ...
    # -----------------------------
    frame_observations = extract_frame_observations_from_dirs(
        frames,
        pose_log,
    )
...
if __name__ == "__main__":
    ...
    landmark_pos, landmark_pts = build_starfield_map_from_logged_data(
        frames_dir="logged_frames",
        poses_dir="logged_poses",
        K=K,
        dist=dist,
        z0=4.5,
        debug=True,
